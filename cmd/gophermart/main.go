...
func main() {
	...
	if err := db.Ping(); err != nil {
		logger.Fatalln("could not ping DB...", err)
	}

	driver, err := postgres.WithInstance(db, &postgres.Config{})
	if err != nil {
		logger.Fatalln("could not start sql migration...", err)
	}

	m, err := migrate.NewWithDatabaseInstance(fmt.Sprintf("file://%s", cfg.MigrationDir), "product", driver)
	if err != nil {
		logger.Fatalln("migration failed... ", err)
	}

	fmt.Println(m.Version())
	if err := m.Up(); err != nil && err != migrate.ErrNoChange {
		logger.Fatalln("An error occurred while syncing the database..", err)
	}
